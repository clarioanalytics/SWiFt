 ...
 */
class GroovyDecisionBuilderTest {
    private static Logger log = LoggerFactory.getLogger(GroovyDecisionBuilderTest.class)

    MockAction s1, s2, s3, s4, s5, s6, s7, s8, s9
    Closure<Action> f1, f2, f3, f4, f5, f6, f7, f8, f9
    List<MockAction> mockActions
    List<Decision> decisions
    GroovyDecisionBuilder builder

    @Before
    void setup() {
        decisions = new ArrayList<>()
        builder = new GroovyDecisionBuilder(decisions)
    ...
    class Replay {
        ...
        List<Map<MockAction, String>> steps = []
        ...
        DecisionState next(Map<MockAction, String> step) {
            ...
            if (step != LAST_STEP) {
                ...
                List<Decision> expectedDecisions = step.keySet().findAll { it != null }.collect { it.getDecision() }.findAll { it != null }
                assertEquals("replay step " + stepCount + " decisions", expectedDecisions, decisions)

                List<String> actualControls = step.keySet().findAll { it != null }.collect { it.getControl() }.findAll { it != null }
    ...
    @Test
    void testFinallyWithErrorNoFailWorkflowAndNoDecisions() {
        f4 = { s4.withInput("finally") }
    ...
    @Test
    void testNoFailWorkflowOnError() {
        s2.setEventStates(NOT_STARTED, ERROR)
        s2.withNoFailWorkflowOnError()
        builder.sequence(f1, f2, f3)
    ...
    @Test
    void testWorklflowCompletionDecision() {
        builder
            .sequence(f1, f2)
    ...
    @Test
    void testSequenceSplitSequenceSplitSequence() {
        f1 = { s1.withInput("") }
        f2 = { s2.withInput(s1.getOutput()) }
        f3 = { s3.withInput(s2.getOutput()) }
        f4 = { s4.withInput(s2.getOutput()) }
